...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $success = updatePost($id, $title, $content, $excerpt, $cover_image, $published, $post_type, $images);
    ...
    $_POST['cover_image'] = $post['cover_image'];
    $_POST['post_type'] = $post['post_type'] ?? 'article';
    $_POST['images'] = $post['images'] ?? '';
    if ($post['published']) {
        $_POST['published'] = '1';
    }
}
...
<head>
    ...
    <title>编辑文章：<?= htmlspecialchars($post['title']) ?> - 好朋友博客</title>
...
<body class="min-h-full bg-white dark:bg-gray-900 font-sans transition-colors duration-300">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <a href="index.php" class="flex items-center space-x-2 text-xl font-bold text-gray-900 hover:text-primary-600 transition-colors">
                    ...
                    <span class="mobile-hidden">好朋友博客</span>
                </a>

                <!-- 导航菜单 -->
                <div class="flex items-center space-x-2 sm:space-x-6">
                    <a href="index.php" class="flex items-center space-x-1 px-2 sm:px-3 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors">
                        <i class="fas fa-home nav-icon"></i>
                        <span class="nav-text">首页</span>
                    </a>
                    
                    <a href="create.php" class="flex items-center space-x-1 px-2 sm:px-3 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors">
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <!-- 返回按钮 -->
        <div class="mb-8">
            <a href="post.php?id=<?= $id ?>" class="inline-flex items-center text-gray-600 hover:text-gray-900 transition-colors group">
        ...
        <form method="POST" class="space-y-8" x-data="{ 
            showPreview: false,
            isPublished: <?= $post['published'] ? 'true' : 'false' ?>,
            autoSave: true,
            lastSaved: null,
            postType: '<?= htmlspecialchars($post['post_type'] ?? 'article') ?>',
            showCoverImage: true,
            showEditor: true
        }">
            <div class="glass rounded-2xl p-4 sm:p-8 mobile-card border border-white/20">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <label class="relative cursor-pointer">
                            <input type="radio" 
                                   ...
                                   value="article" 
                                   x-model="postType"
                                   <?= ($_POST['post_type'] ?? 'article') === 'article' ? 'checked' : '' ?>
                                   class="sr-only peer">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" 
                                   ...
                                   value="image" 
                                   x-model="postType"
                                   <?= ($_POST['post_type'] ?? 'article') === 'image' ? 'checked' : '' ?>
                                   class="sr-only peer">
                            <div class="p-4 rounded-lg border-2 border-gray-200 peer-checked:border-primary-500 peer-checked:bg-primary-50 transition-all">
                                <div class="flex items-center space-x-3">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" 
                                   ...
                                   value="text" 
                                   x-model="postType"
                                   <?= ($_POST['post_type'] ?? 'article') === 'text' ? 'checked' : '' ?>
                                   class="sr-only peer">
                            <div class="p-4 rounded-lg border-2 border-gray-200 peer-checked:border-primary-500 peer-checked:bg-primary-50 transition-all">
                                <div class="flex items-center space-x-3">
                ...
                <div class="mb-8" x-show="postType === 'image'" x-data="{ 
                    selectedImages: <?= json_encode(json_decode($_POST['images'] ?? '[]', true) ?: []) ?>, 
                    dragOver: false,
                    uploading: false
                }">
                    <label class="block text-sm font-semibold text-gray-700 mb-3">
                        <i class="fas fa-images mr-2 text-primary-500"></i>
                        图片管理 <span class="text-red-500">*</span>
                    </label>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center"
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <p class="text-lg font-medium text-gray-700">添加更多图片</p>
                        ...
                        <button type="button" 
                            ...
                            <span x-text="uploading ? '上传中...' : '添加图片'"></span>
                        </button>
                    </div>
                ...
                <div class="mb-8">
                    ...
                    <div x-show="postType === 'image'">
                        ...
                        <div class="mt-2 flex items-center justify-between text-sm text-gray-500">
                            ...
                            <span id="char-count-edit">0/200</span>
                        </div>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-6 border-t border-gray-200 space-y-4 sm:space-y-0">
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center space-x-2 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-gray-700">发布文章</span>
                        </label>
    ...
    <script>
        ...
        if (!window.selectedImages) {
            ...
            // 加载现有图片数据
            const existingImages = <?= json_encode($post['images'] ?? '[]') ?>;
            if (existingImages && existingImages !== '[]') {
                try {
                    const imageData = JSON.parse(existingImages);
                    if (Array.isArray(imageData)) {
                        window.selectedImages = imageData;
                        // 更新隐藏的input值
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const charCountElement = document.getElementById('char-count-edit');
            
            if (imageTextarea && charCountElement) {
                // 初始计算
                const initialLength = imageTextarea.value.length;
                charCountElement.textContent = initialLength + '/200';
